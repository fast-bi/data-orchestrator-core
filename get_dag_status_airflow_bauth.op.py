...
#BASIC AUTH STARTS
def basicauthapi_get_dag_status(airflow_dag_id, airflow_user, airflow_pass):
    # Set the API endpoint, auth, and headers
    url = airflow_url+'/api/v1/dags/'+airflow_dag_id+'/dagRuns/'+dag_run_id
    ...
    if response.status_code == 200:
        print("DAG "+airflow_dag_id+" status found.")
    else:
    ...
    if state is not None:
        print("DAG", airflow_dag_id+" Running state:", state)
    else:
        raise Exception("Key 'state' not found in response JSON")
    ...
    if state == "success":
        print("DAG "+airflow_dag_id+" status for run "+dag_run_id+" "+state+".")
        print("Status:", state)
    elif state == "failed":
        print("DAG "+airflow_dag_id+" status for run "+dag_run_id+" "+state+".")
        print("Status:", state)
        ...
        print("DAG "+airflow_dag_id+" status for run "+dag_run_id+" "+state+" unknown.")
        print("Status:", state)
        sys.exit(1)
#BASIC AUTH ENDS
...
def googleauth_get_dag_status(airflow_dag_id, dag_run_id, client_id):
    ...
    if use_experimental_api:
        endpoint = f'api/experimental/dags/'+airflow_dag_id+'/dagRuns/'+dag_run_id
    else:
        endpoint = f'/api/v1/dags/'+airflow_dag_id+'/dagRuns/'+dag_run_id
...
if platform.lower() == "airflow":
    basicauthapi_get_dag_status(airflow_dag_id, airflow_user, airflow_pass)
elif platform.lower() == "composer":
    client_id = get_client_id(project_id, location, composer_environment)
    googleauth_get_dag_status(airflow_dag_id, dag_run_id, client_id)
