...
airflow_vars = define_env_variables()
DAG_FOLDER_NAME = airflow_vars.get("DBT_PROJECT_NAME")

if airflow_vars.get("TARGET") == 'test':
    DBT_FOLDER_NAME = DAG_FOLDER_NAME + '_' + airflow_vars.get("GIT_BRANCH")
else:
    DBT_FOLDER_NAME = DAG_FOLDER_NAME
...
dag_parser = DbtManifestParser(
    ...
    dbt_tag=DBT_TAG,
    dbt_project_dir=DBT_FOLDER_NAME,
    airflow_vars=airflow_vars,
    debug=airflow_vars.get("DEBUG", "").lower() == "true",  # Get DEBUG flag from Airflow variables
...
with models.DAG(
    ...
    def f_show_input_data(session=None, **kwargs):
        ...
        if "full_refresh" in dag_run.conf:
            ...
            kwargs["ti"].xcom_push(key="full_refresh_model", value=full_refresh_value)
            kwargs["ti"].xcom_push(key="full_refresh_source", value=full_refresh_value)
            kwargs["ti"].xcom_push(key="full_refresh_seed", value=full_refresh_value)
            kwargs["ti"].xcom_push(key="full_refresh_snapshot", value=full_refresh_value)
    ...
    if airflow_vars.get("DEBUG", "").lower() == "true":
        dbt_debug_task = dag_parser.create_dbt_bash_task(
            dbt_command="debug",
            node_name="check_datawarehouse_connection",
    ...
    # Batch-mode snapshot dependency handling.
    # When models run as a single batch (DBT_MODEL_SHARDING=false) a plain
    # `dbt run` cannot include snapshots, so a `model -> snapshot -> model` chain
    # would run the downstream model before the snapshot. If DBT_MODEL_DEPENDS_ON_SNAPSHOT
    # is enabled and a model actually depends on a snapshot, we replace the separate
    # model-run + snapshot batches with a single `dbt build` (which orders models,
    # snapshots and tests by lineage in one process). Sharded mode is unaffected.
    use_build_batch = (
        DBT_MODEL == "true"
    ...
    if dag_parser.is_resource_type_in_manifest("model"):
        if DBT_MODEL == "true":
            ...
            log.info("dbt model not enabled.")
    ...
    if DATA_QUALITY == "true":
        re_data_models = dag_parser.create_dbt_bash_task(dbt_command="re_data",
