...
airflow_secret_file_name = get_env_var('airflow_secret_file_name', 'Airflow/Composer Secret Variables file name')
...
#Read The YML Variables file
def yml2json(airflow_secret_file_name):
    with open(airflow_secret_file_name, 'r') as file:
        configuration = yaml.safe_load(file)

    #output = json.dumps(configuration)
    key_name = list(configuration.keys())[0]
    value_json = json.dumps(*configuration.values())

    if not re.search('secret', key_name.lower()):
        key_name = key_name+'_SECRET'

    # # Define the variable to create
    data = {"key": key_name, "value": value_json}

    return data

def basicauthapi(data, airflow_user, airflow_pass):
    # Set the API endpoint, auth, and headers
    url = airflow_url+'/api/v1/variables'
    ...
    if response.status_code == 200:
        ...
        print("Error creating variable. Response code: ", response.status_code)
        print(response.text)
...
def make_iap_request(url, client_id, method='GET', **kwargs):
    ...
    if resp.status_code == 403:
        ...
        print("Error creating variable. Response code: ", resp.status_code)
        return resp.text

def add_variables(data, context=None):
    ...
    if USE_EXPERIMENTAL_API:
        endpoint = f'api/experimental/variables'
        json_data = data
    else:
        endpoint = f'/api/v1/variables'
        json_data = data
...
if platform.lower() == "airflow":
    data = yml2json(airflow_secret_file_name)
    basicauthapi(data, airflow_user, airflow_pass)
